...
function initialize(trips,timeStep,startDate,endDate){
	...
	var stocks = getStocksFromTripsAndInitialLevels(trips, minutesRange);
	var vehicles = getVehiclesFromTrips(trips);

	setFlowRatesFromTrips_v1(trips, stocks, minutesRange);
	decomposeSeasonalityTrend_v1(stocks);
	integrateLevels_v1(stocks);
	addStats(stocks);
